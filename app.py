...
st.markdown(
    f"<div class='hero-title'>{PROJECT_CONFIG['project']['name']}</div>",
    unsafe_allow_html=True,
)
st.markdown(
    "<div class='hero-subtitle'>"
    "Interactive dashboard for churn behavior, live customer scoring, and final model diagnostics."
    "</div>",
    unsafe_allow_html=True,
)
...
st.markdown(
    ...
    "First-look business view: how churn is distributed, where it concentrates, "
    "and which customer segments deserve attention."
    "</div>",
    unsafe_allow_html=True,
)
...
with row1_col2:
    ...
    fig.update_yaxes(
        ...
        range=[0, geo_churn["Churn Rate"].max() * 1.18],
    )
    st.plotly_chart(fig, use_container_width=True)
...
with row1_col3:
    ...
    fig.update_yaxes(
        ...
        gridcolor=GRID_COLOR,
        tickfont=dict(color=MUTED_TEXT),
        range=[0, gender_churn["Churn Rate"].max() * 1.22],
    )
    st.plotly_chart(fig, use_container_width=True)
...
with row2_col3:
    ...
    fig.update_yaxes(
        ...
        gridcolor=GRID_COLOR,
        tickfont=dict(color=MUTED_TEXT),
        range=[0, product_churn["Churn Rate"].max() * 1.20],
    )
    st.plotly_chart(fig, use_container_width=True)
...
st.markdown(
    "<div class='section-copy'>"
    "Enter one customer profile, score it with the deployed model, and inspect the churn probability "
    "against the saved operating threshold."
    "</div>",
    unsafe_allow_html=True,
)
...
st.markdown(
    "<div class='section-title'>3. Model Overview &amp; Technical Details</div>",
    unsafe_allow_html=True,
)
st.markdown(
    "<div class='section-copy'>"
    "Compares trained models directly and shows the technical summary behind the deployed choice."
    "</div>",
    unsafe_allow_html=True,
)
...
def metric_bar(metric_key, title, color_scale):
    plot_df = results_df[["model_display", metric_key]].sort_values(metric_key, ascending=False)
    fig = px.bar(
        plot_df,
        x="model_display",
        y=metric_key,
    ...
    fig.update_xaxes(showgrid=False, tickfont=dict(color=TEXT_COLOR))
    fig.update_yaxes(range=[0, 1], gridcolor=GRID_COLOR, tickfont=dict(color=MUTED_TEXT))
    return fig


with metric_row1_col1:
    st.plotly_chart(metric_bar("recall", "Recall Comparison", WARM_SCALE), use_container_width=True)
with metric_row1_col2:
    st.plotly_chart(metric_bar("precision", "Precision Comparison", WARM_SCALE), use_container_width=True)
with metric_row1_col3:
    st.plotly_chart(metric_bar("f1_score", "F1 Score Comparison", WARM_SCALE), use_container_width=True)

with metric_row2_col1:
    st.plotly_chart(metric_bar("accuracy", "Accuracy Comparison", WARM_SCALE), use_container_width=True)
with metric_row2_col2:
    st.plotly_chart(metric_bar("roc_auc", "ROC-AUC Comparison", WARM_SCALE), use_container_width=True)
with metric_row2_col3:
    st.plotly_chart(metric_bar("cv_f1_mean", "Cross-Validation F1", RISK_SCALE), use_container_width=True)

detail_col1, detail_col2 = st.columns([1.05, 1])
        ...
        [
            {
                ...
                "Selected Threshold": f"{best_threshold:.2f}",
                "Accuracy": f"{best_result['accuracy']:.3f}",
                "Precision": f"{best_result['precision']:.3f}",
                "Recall": f"{best_result['recall']:.3f}",
                "F1 Score": f"{best_result['f1_score']:.3f}",
                "ROC-AUC": f"{best_result['roc_auc']:.3f}",
                "CV F1 Mean": f"{best_result['cv_f1_mean']:.3f}",
            }
        ]
    )
...
with detail_col2:
    ...
    confusion_values = exp.get("confusion_matrix", [])
    ...
    if confusion_values:
        conf_fig = go.Figure(
            data=go.Heatmap(
                ...
                colorscale=[[0, "#0d0d0d"], [0.5, "#9f8630"], [1, ACCENT_COLOR]],
...
st.markdown(
    ...
    <div class='small-note'>
        Technical note: live predictions use the saved deployed model <strong>{best_model_display}</strong>
        and threshold <strong>{best_threshold:.2f}</strong>. Other models are shown for comparison only.
    </div>
    """,
    unsafe_allow_html=True,
)
